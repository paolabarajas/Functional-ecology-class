...
# R version 4.5.2 (2025-10-31 ucrt)  R.Version()
# Last code update: 23 Feb 2026
# _____________________________________________________________

# Install the packages if necessary by using install.packages command
...
# # 
# 1. Plots Figure (Barajas Barbosa et al 2023)
Tenerife <- PCAvalues %>%
  ggplot(aes(PC1, PC2)) +   
  stat_density_2d(
    geom = "polygon",
  ...
  geom_jitter(alpha = 0.6, size = 2, colour = "turquoise4") +
  geom_text(
    data = PCAloadings,
...
MAC <- MAC  %>% ggplot(aes(PC1,  PC2))+ 
  stat_density_2d(geom = "polygon", contour = TRUE, aes(fill = after_stat(level)), colour = "gray", bins = 10) +
  scale_fill_distiller(palette = "Greys", direction = 1) +
  geom_jitter(alpha=0.5,  size = 2  , colour = "green4") +    
  xlim(-5 , 5) + ylim(-5, 4) +
  xlab("n = 54") + ylab("")   + theme_minimal()
MAC
...
ggpubr::ggarrange(Ten, ns, mac, sie, ncol =2, nrow = 2) 

# Estimating functional diversity metrics Trait-value-based approach 
...
hv_list$hv_ae <-  hypervolume_gaussian(
